...
export interface GenerateOptions {
  ...
  // When provided, prepends a provenance comment naming the helper version
  // and the hosted URL above the <nav>. Lets a future reader find the tool
  // that generated the markup.
...
export function generateSidenavHtml(
  ...
  const versionTag = options.helperVersion ? ` v${options.helperVersion}` : ''
  const provenance = `<!-- Generated by sidenavHelper${versionTag} — https://darrenolah.github.io/sidenavHelper/ -->\n`

  return `${provenance}<nav class="au-sidenav" aria-label="${header}">

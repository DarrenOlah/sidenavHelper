...
describe('generateSidenavHtml', () => {
  it('emits a <nav class="au-sidenav"> wrapper with default header text', () => {
    ...
    expect(html).toContain('<nav class="au-sidenav" aria-label="In this section">')
  ...
  describe('provenance comment', () => {
    it('prepends a provenance comment with no version when helperVersion is not provided', () => {
      const html = generateSidenavHtml([leaf('1', 'A', '/a/')])
      expect(html.startsWith('<!-- Generated by sidenavHelper — https://darrenolah.github.io/sidenavHelper/ -->\n')).toBe(true)
    })

    it('includes the version when helperVersion is provided', () => {
      const html = generateSidenavHtml([leaf('1', 'A', '/a/')], { helperVersion: '0.0.0' })
      expect(html.startsWith('<!-- Generated by sidenavHelper v0.0.0 — https://darrenolah.github.io/sidenavHelper/ -->\n')).toBe(true)
    })

    it('appears exactly once in the output', () => {
      const html = generateSidenavHtml([leaf('1', 'A', '/a/')], { helperVersion: '1.2.3' })
      expect(html.match(/Generated by sidenavHelper/g) ?? []).toHaveLength(1)
    })
  })
